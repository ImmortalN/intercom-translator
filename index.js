...
// Актуальные живые инстансы (20 ноября 2025)
const PRIMARY_TRANSLATE_API_URL = 'https://libretranslate.de/translate';  // №1, стабильный
const FALLBACK_TRANSLATE_APIS = [
  'https://translate.argosopentech.com/translate',  // №2, Argos Translate (поддерживает иврит)
  'https://translate.fedilab.app/translate'  // №3, резерв (если хочешь, но с мусором — код заблокирует)
];
...
const LANG_MAP = {
  ...
  'ar': 'ar', 'zh': 'zh', 'zh-Hant': 'zh', 'zh-Hans': 'zh', 'ko': 'ko', 'ja': 'ja', 'he': 'he',
  ...
  'Traditional Chinese': 'zh-Hant', 'Hebrew': 'he'
};
...
function isGarbageTranslation(text) {
  ...
  if (text.length > 300 && text.split(' ').length < 15) return true;
  return false;
}
...
app.post('/intercom-webhook', async (req, res) => {
  ...
  try {
    ...
    TRANSLATION_CACHE.set(cacheKeyPrev, messageText);
...
async function translateMessage(text, detectedLang) {
  ...
  async function tryMyMemoryWithRetry(q, source) {
    ...
    // ← УЛУЧШЕНО: Для иврита используем 'he|en' вместо 'auto'
    const langpair = (source === 'he' || /[\u0590-\u05FF]/.test(q)) ? 'he|en' : (source === 'auto' ? 'auto|en' : `${source}|en`);
    const paramsBase = { langpair };
